...
/**
 * Faixa que avisa ao visitante — o advogado avaliando o modelo — que todo o
 * conteúdo é personalizável.
 *
 ...
 * quebra em duas linhas no celular ou quando a faixa é fechada).
 *
...
export default function TemplateBar({ onHeightChange }) {
  ...
  return (
    <div ref={ref} className="fixed inset-x-0 top-0 z-50 border-b border-gold-400/25 bg-ink-950">
      <div className="container-x flex items-center justify-between gap-3 py-2">
        <p className="flex items-start gap-2.5 text-[12.5px] leading-snug text-bone-200 sm:items-center">
          <PencilRuler size={15} className="mt-0.5 shrink-0 text-gold-300 sm:mt-0" aria-hidden />
          <span>
            <strong className="font-bold text-bone-50">Modelo demonstrativo.</strong> Textos, cores,
            fotos e áreas de atuação são personalizáveis com os dados do seu escritório.
          </span>
        </p>
        ...
        <button
          ...
          className="-mr-2 flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-sm
                     text-bone-300 transition-colors duration-200 hover:bg-white/10 hover:text-bone-50"
        >
