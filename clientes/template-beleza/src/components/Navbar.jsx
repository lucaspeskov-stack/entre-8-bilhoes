 ...
 *
 * `offset` recebe a altura da faixa do modelo para o header descer junto.
 */
  ...
  return (
    <header
      ...
      className={`fixed inset-x-0 z-50 transition-all duration-300 ${
        solid
          ? 'border-b border-marfim-300 bg-marfim-50/95 shadow-soft backdrop-blur-md'
          : 'border-b border-transparent bg-gradient-to-b from-noite-950/60 to-transparent'
      }`}
    >
      <nav
            ...
            <a
              ...
              className={`whitespace-nowrap rounded-full px-3.5 py-2 text-sm font-medium
                          transition-colors duration-200 ${
                            solid
                              ? 'text-noite-500 hover:bg-marfim-200 hover:text-noite-900'
                              : 'text-marfim-100 hover:bg-white/10 hover:text-white'
                          }`}
            >
              {l.label}
            </a>
        ...
        <div className="flex items-center gap-2">
          <a
            ...
            /* Visível do tablet até `xl`, onde o menu completo assume a barra.
               ...
               rodapé — repeti-lo aqui espremeria o nome do salão. */
            className={`hidden min-h-[44px] items-center gap-2 whitespace-nowrap rounded-full px-3
                        text-sm font-medium transition-colors duration-200
                        md:inline-flex xl:hidden ${
                          solid
                            ? 'text-noite-800 hover:text-champanhe-700'
                            : 'text-marfim-100 hover:text-white'
                        }`}
          >
          ...
          <a
            href="#contato"
            className="btn-primary hidden !min-h-[44px] whitespace-nowrap !px-6 !py-2.5 sm:inline-flex"
          >
            Agendar
          </a>
          ...
          <button
            ...
            className={`inline-flex h-11 w-11 cursor-pointer items-center justify-center rounded-full
                        transition-colors duration-200 xl:hidden ${
                          solid
                            ? 'text-noite-900 hover:bg-marfim-200'
                            : 'text-marfim-50 hover:bg-white/10'
                        }`}
          >
      ...
      {open && (
        <div
          id="menu-mobile"
          className="animate-fade-down border-t border-marfim-200 bg-marfim-50 xl:hidden"
        >
          <div className="container-x flex flex-col py-3">
            {navLinks.map((l) => (
              <a
                ...
                className="flex min-h-[48px] items-center border-b border-marfim-200 text-[15px]
                           font-medium text-noite-700 transition-colors duration-200 last:border-b-0
                           hover:text-champanhe-700"
              >
                {l.label}
              </a>
            ))}
